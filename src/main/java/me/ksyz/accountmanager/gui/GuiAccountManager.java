...
public class GuiAccountManager extends GuiScreen {
  ...
  class GuiAccountList extends GuiSlot {
    ...
    protected void drawSlot(int entryID, int x, int int_4, int k, int mouseXIn, int mouseYIn) {
      ...
      long unbanTimeMMC = account.getUnbanMMC();
      String unban, unbanMMC;
      if (unbanTime < 0L) {
        unban = "&4&l⚠";
      ...
      if (unbanTimeMMC < 0L) {
        unbanMMC = "&d&l⚠";
      } else if (unbanTimeMMC <= currentTime) {
        unbanMMC = "&a&l✔";
      } else {
        long diff = unbanTimeMMC - currentTime;
        long s = (diff / 1000L) % 60L;
        long m = (diff / 60000L) % 60L;
        long h = (diff / 3600000L) % 24L;
        long d = (diff / 86400000L);
        unbanMMC = String.format(
                "%s%s%s%s",
                d > 0L ? String.format("%dd", d) : "",
                h > 0L ? String.format(" %dh", h) : "",
                m > 0L ? String.format(" %dm", m) : "",
        ...
        unbanMMC = unbanMMC.trim();
        unbanMMC = String.format("%s &c&l⚠", unbanMMC);
      }
      ...
      unbanMMC = TextFormatting.translate(
              String.format("&r%s&r", unbanMMC)
      );
